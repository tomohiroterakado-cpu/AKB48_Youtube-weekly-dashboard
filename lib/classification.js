...
function classifyPublication(value) {
  const text = String(value || "").trim();
  if (!text) return { date: null, weekday: "未取得", time: null, timeSlot: "未取得", reason: "CSVに公開日時がありません", confidence: 0, needsReview: true, source: "system_auto" };
  const hasTime = /\d{1,2}:\d{2}/.test(text) || /T\d{2}:\d{2}/.test(text);
  const hasZone = /(?:Z|[+-]\d{2}:?\d{2}|JST|GMT|UTC)/i.test(text);
  const parseTarget = hasTime && !hasZone
    ? (text.includes("T") ? `${text}+09:00` : `${text} JST`)
    : text;
  const date = new Date(parseTarget);
  if (Number.isNaN(date.getTime())) return { date: text, weekday: "未判定", time: null, timeSlot: "未判定", reason: "公開日時を解釈できません", confidence: 20, needsReview: true, source: "system_auto" };
  const parts = Object.fromEntries(TOKYO_PARTS.formatToParts(date).filter((part) => part.type !== "literal").map((part) => [part.type, part.value]));
  const hours = hasTime ? Number(parts.hour) : null;
  const timeSlot = hours === null ? "未取得" : hours < 6 ? "深夜" : hours < 12 ? "午前" : hours < 17 ? "昼" : hours < 21 ? "夜" : "深夜";
  const weekday = new Intl.DateTimeFormat("ja-JP", { timeZone: "Asia/Tokyo", weekday: "short" }).format(date);
  return {
    date: `${parts.year}-${parts.month}-${parts.day}`,
    weekday: WEEKDAYS.indexOf(weekday) >= 0 ? weekday : "未判定",
    time: hasTime ? `${parts.hour}:${parts.minute}` : null,
    timeSlot,
    reason: hasTime ? "CSV公開日時から算出" : "CSV公開日から曜日を算出。時刻は未取得",
    confidence: hasTime ? 95 : 80,
    needsReview: !hasTime,
    source: "system_auto"
  };
}
...
function classifyVideo(video, memberNames = []) {
  ...
  return {
    ...
    publication: classifyPublication(video.publishedAt),
    titleFeatures,
...
module.exports = { classifyFormat, classifyGenre, classifyPublication, classifyVideo, detectMembers, extractTitleFeatures };
