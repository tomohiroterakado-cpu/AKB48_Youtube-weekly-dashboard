...
const CONTENT_METRIC_HEADERS = [
  { header: "チャンネル登録者", aliases: ["チャンネル登録者", "チャンネル登録者数"] },
  { header: "推定収益 (JPY)", aliases: ["推定収益 (JPY)", "推定収益（JPY）", "推定収益"] },
  { header: "コメントの追加回数", aliases: ["コメントの追加回数", "コメント追加回数", "コメント数"] }
];

function tablePlan({ sheet, headerRow = 3, requiresWeekStart = true, requiredHeaders = [], keyForRecord, keyForRow, records, apply, derive }) {
  return { sheet, headerRow, requiresWeekStart, requiredHeaders, keyForRecord, keyForRow, records, apply, derive };
}
...
function buildPlans(state, importRecord, dailyImport) {
  ...
  const applyWeekly = (row, columns, record) => {
    ...
    setAliases(row, columns, ["チャンネル登録者", "チャンネル登録者数"], metricValue(record.summary, "subscribers"));
    setAliases(row, columns, ["登録者増加数", "チャンネル登録者増加数"], metricValue(record.summary, "subscriberGains"));
    setAliases(row, columns, ["推定収益 (JPY)", "推定収益（JPY）", "推定収益"], metricValue(record.summary, "estimatedRevenue"));
    ...
    setAliases(row, columns, ["コメントの追加回数", "コメント追加回数", "コメント数"], metricValue(record.summary, "comments"));
  };
  ...
  const applyContent = (row, columns, record) => {
    ...
    setAliases(row, columns, ["コメントの追加回数", "コメント追加回数", "コメント数"], metricValue(values, "comments"));
    ...
    setAliases(row, columns, ["チャンネル登録者", "チャンネル登録者数"], metricValue(values, "subscribers"));
    setAliases(row, columns, ["登録者増加数", "チャンネル登録者増加数"], metricValue(values, "subscriberGains"));
    setAliases(row, columns, ["推定収益 (JPY)", "推定収益（JPY）", "推定収益"], metricValue(values, "estimatedRevenue"));
  };
  ...
  return [
    tablePlan({ sheet: "CSV_週次集計", requiredHeaders: CONTENT_METRIC_HEADERS, keyForRecord: (record) => dateKey(record.periodStart), keyForRow: (row, columns) => dateKey(row[findColumn(columns, ["週開始日"])]), records: [weekly], apply: applyWeekly, derive: deriveWeeklyCumulative }),
    tablePlan({ sheet: "CSV_貼付用", requiredHeaders: CONTENT_METRIC_HEADERS, keyForRecord: (record) => `${dateKey(importRecord.periodStart)}_${record.metric.videoId}`, keyForRow: (row, columns) => `${dateKey(row[findColumn(columns, ["週開始日"])] || "")}_${videoKey(row[findColumn(columns, ["動画ID", "コンテンツ", "URL"])] || "")}`, records: contentRecords, apply: applyContent }),
    tablePlan({ sheet: "自チャンネル動画", requiredHeaders: CONTENT_METRIC_HEADERS, keyForRecord: (record) => `${dateKey(importRecord.periodStart)}_${record.metric.videoId}`, keyForRow: (row, columns) => `${dateKey(row[findColumn(columns, ["週開始日"])] || "")}_${videoKey(row[findColumn(columns, ["動画ID", "コンテンツ", "URL"])] || "")}`, records: contentRecords, apply: applyContent }),
...
async function syncPlan(repository, plan, preloadedResult = null) {
  const result = preloadedResult || await repository.readRange(`${plan.sheet}!A:ZZ`);
  const rows = result.values || [];
  const originalHeaders = rows[plan.headerRow - 1] || [];
  if (!originalHeaders.length) throw new Error(`${plan.sheet} の見出し行が見つかりません。`);
  const additions = plan.requiredHeaders.filter((item) => findColumn(columnsFor(originalHeaders), item.aliases) < 0);
  const headers = [...originalHeaders, ...additions.map((item) => item.header)];
  ...
  if (additions.length) {
    await repository.batchWriteRanges([{ range: `${plan.sheet}!${columnName(originalHeaders.length)}${plan.headerRow}`, values: [additions.map((item) => item.header)] }]);
  }
  if (writes.length) await repository.batchWriteRanges(writes);
  return { sheet: plan.sheet, inserted, updated, records: plan.records.length, addedHeaders: additions.map((item) => item.header) };
}
...
async function syncLegacyWeeklyReport(repository, { periodStart, periodEnd } = {}) {
  ...
  const plans = buildPlans(state, importRecord, dailyImport);
  const loadedRows = typeof repository.readRanges === "function"
    ? await repository.readRanges(plans.map((plan) => `${plan.sheet}!A:ZZ`))
    : await Promise.all(plans.map((plan) => repository.readRange(`${plan.sheet}!A:ZZ`)));
  const results = [];
  for (const [index, plan] of plans.entries()) results.push(await syncPlan(repository, plan, loadedRows[index]));
