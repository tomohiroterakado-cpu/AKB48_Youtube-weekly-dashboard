...
function tablePlan({ sheet, headerRow = 3, keyForRecord, keyForRow, records, apply }) {
  return { sheet, headerRow, keyForRecord, keyForRow, records, apply };
}
...
function buildPlans(state, importRecord, dailyImport) {
  ...
  return [
    tablePlan({ sheet: "CSV_週次集計", keyForRecord: (record) => dateKey(record.periodStart), keyForRow: (row, columns) => dateKey(row[findColumn(columns, ["週開始日"])]), records: [weekly], apply: applyWeekly }),
    ...
    tablePlan({ sheet: "CSV_日別", keyForRecord: (record) => `${dateKey(importRecord.periodStart)}_${dateKey(record.date)}`, keyForRow: (row, columns) => `${dateKey(row[findColumn(columns, ["週開始日"])] || "")}_${dateKey(row[findColumn(columns, ["日付", "日"])] || "")}`, records: dailyMetrics, apply: applyDaily })
  ];
}
...
async function syncPlan(repository, plan) {
  ...
  if (findColumn(columns, ["週開始日"]) < 0) throw new Error(`${plan.sheet} に「週開始日」列が見つかりません。`);
