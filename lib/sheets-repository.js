const TABLES = {
  ...
  thumbnailGenerations: { sheet: "AI_thumbnail_generations", fields: ["id", "fingerprint", "createdAt", "expiresAt", "status", "completedAt"] },
  prepublishReviews: { sheet: "AI_prepublish_reviews", fields: ["id", "createdAt", "channel", "seriesName", "episode", "targetAudience", "proofPoints", "currentTitle", "recommendedTitle", "abTitles", "estimatedCtr", "status", "adoptedTitle", "publishedVideoId", "actualCtr", "measuredAt", "analysisVersion", "model"] }
};

const JSON_FIELDS = new Set(["missingCounts", "missingDates", "missingMetricColumns", "unknownHeaders", "summary", "members", "guests", "tags", "values", "aliases", "data", "abTitles"]);
...
class GoogleSheetsRepository {
  ...
  async readFresh({ trackRowIndexes = false } = {}) {
    ...
    const state = { schemaVersion: 1, imports: [], dailyImports: [], videos: [], metrics: [], dailyMetrics: [], classifications: [], reviews: [], members: [], categories: [], marketReports: [], thumbnailGenerations: [], prepublishReviews: [], settings: {} };
    const rowIndexes = {};
