  ...
  async ensureSchema() {
    ...
    if (missing.length) {
      await this.request(`${base}:batchUpdate`, {
        method: "POST",
        body: JSON.stringify({ requests: missing.map((table) => ({ addSheet: { properties: { title: table.sheet, frozenRowCount: 1 } } })) })
      });
    }
    ...
    for (const [name, table] of Object.entries(TABLES)) {
      ...
      for (const item of draft[name]) {
        ...
        if (!previous) additions.push(row);
          ...
          await this.writeRange(`${table.sheet}!A${rowNumber}`, [row]);
        }
      }
      await this.appendRows(table.sheet, additions);
    }
    return result;
  }
}
