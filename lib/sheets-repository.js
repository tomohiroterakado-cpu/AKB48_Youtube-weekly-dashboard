...
const JSON_FIELDS = new Set(["missingCounts", "missingDates", "missingMetricColumns", "unknownHeaders", "summary", "members", "guests", "tags", "values", "aliases"]);
...
class GoogleSheetsRepository {
  ...
  async ensureSchema() {
    ...
    for (const table of Object.values(TABLES)) {
      if (!existingByTitle.has(table.sheet)) {
        await this.writeRange(`${table.sheet}!A1`, [table.fields]);
        continue;
      }
      const headerRow = await this.readRange(`${table.sheet}!1:1`);
  ...
  async read() {
    ...
    const state = { schemaVersion: 1, imports: [], dailyImports: [], videos: [], metrics: [], dailyMetrics: [], classifications: [], reviews: [], members: [], categories: [], settings: {} };
    this.rowIndexes = {};
...
module.exports = { GoogleSheetsRepository, TABLES, cellValue, parseValue, spreadsheetColumn };
