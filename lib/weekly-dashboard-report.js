  ...
  const weeks = latestImportsByPeriod(state).map((importRecord) => {
    ...
    const topVideos = [...metrics].sort((left, right) => metricValue(right, "views") - metricValue(left, "views")).slice(0, 4).map((metric) => {
      ...
      return {
        ...
        avg: values.averagePercentageViewed !== null && values.averagePercentageViewed !== undefined ? `視聴率 ${values.averagePercentageViewed}%` : "",
...
module.exports = { buildWeeklyDashboardData, isLongFormPublishedInPeriod, publishedDateKey, videoAdvice };
