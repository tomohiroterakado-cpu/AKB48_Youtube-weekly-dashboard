...
function optionalMetricValue(metric, key) {
...
function videoAdvice(values) {
  const ctr = Number(values?.ctr || 0);
  const comments = Number(values?.comments || 0);
  const subscriberGains = Number(values?.subscriberGains || 0);
  const estimatedRevenue = Number(values?.estimatedRevenue || 0);
  if (ctr >= 8) return "CTRが高い動画です。タイトルとサムネイルの見せ方を、次回の類似企画でも検証します。";
  if (comments >= 150) return "コメント反応が強い動画です。反応が集まった話題をShortsやコミュニティ投稿にも横展開します。";
  if (subscriberGains > 0) return "登録者増加につながっています。固定コメントと終了画面の導線を同じ型で継続検証します。";
  if (estimatedRevenue > 0) return "推定収益は参考値として記録し、次週以降に同じ公開後経過日数で比較します。";
  return "次週以降の推移と比較して、再現できる要素を確認します。";
}

function isExcludedFromTopVideos(video) {
  const visibility = String(video?.visibility || "").trim().toLowerCase();
  return /限定公開|非公開|unlisted|private/.test(visibility);
}

function selectTopVideos(metrics, videosById) {
  const selectedIds = new Set();
  return [...metrics]
    .sort((left, right) => metricValue(right, "views") - metricValue(left, "views"))
    .filter((metric) => {
      const videoId = String(metric?.videoId || "");
      if (!videoId || selectedIds.has(videoId)) return false;
      if (isExcludedFromTopVideos(videosById.get(videoId))) return false;
    ...
    .map((metric) => {
      ...
      return {
        ...
        publishDate: publishedDateKey(video.publishedAt),
        durationSeconds: Number(video.durationSeconds || values.durationSeconds || 0),
        ...
        subscribers: optionalMetricValue(metric, "subscribers"),
        subscriberGains: optionalMetricValue(metric, "subscriberGains"),
        estimatedRevenue: optionalMetricValue(metric, "estimatedRevenue"),
        ctr: Number.isFinite(Number(values.ctr)) ? `${Number(values.ctr)}%` : "",
        advice: videoAdvice(values),
        memo: ""
      };
    });
}

function strategicVideoMetric(metric, video = {}) {
  const values = metric?.values || {};
  return {
    id: String(metric?.videoId || ""),
    title: video.title || metric?.videoId || "",
    publishedAt: publishedDateKey(video.publishedAt),
    durationSeconds: Number(video.durationSeconds || values.durationSeconds || 0),
    format: video.format || "",
    genre: video.genre || "",
    members: Array.isArray(video.members) ? video.members : [],
    tags: Array.isArray(video.tags) ? video.tags : [],
    visibility: video.visibility || "",
    metricWindow: "対象週7日間",
    views: optionalMetricValue(metric, "views"),
    impressions: optionalMetricValue(metric, "impressions"),
    ctr: optionalMetricValue(metric, "ctr"),
    newViewers: optionalMetricValue(metric, "newViewers"),
    returningViewers: optionalMetricValue(metric, "returningViewers"),
    averageViewDuration: values.averageViewDuration || "",
    likes: optionalMetricValue(metric, "likes"),
    comments: optionalMetricValue(metric, "comments"),
    subscriberGains: optionalMetricValue(metric, "subscriberGains")
  };
}
...
function buildWeeklyDashboardData(state) {
  ...
  const weeks = latestImportsByPeriod(state).map((importRecord) => {
    ...
    const topVideos = selectTopVideos(metrics, videosById);
    const videoMetrics = metrics
      .map((metric) => strategicVideoMetric(metric, videosById.get(metric.videoId)))
      .filter((metric) => metric.id && !isExcludedFromTopVideos(metric));
    return {
...
module.exports = { buildWeeklyDashboardData, isExcludedFromTopVideos, isLongFormPublishedInPeriod, publishedDateKey, selectTopVideos, strategicVideoMetric, videoAdvice };
