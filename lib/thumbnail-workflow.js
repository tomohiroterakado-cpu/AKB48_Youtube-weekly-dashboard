...
function createThumbnailReview(input) {
  ...
  return {
    ...
    protection: {
      ...
      textStrategy: "caption_safe_area_then_browser_composite"
    },
    candidates: CANDIDATES.map((candidate) => ({
      ...candidate,
      recommendedCopy: requestedCopy,
      generationScope: ["背景", "装飾", "金属感", "光沢", "テロップ用安全領域"],
...
function selectThumbnailCandidate(review, candidateId) {
  ...
  return {
    ...
    images2Brief: {
      ...
      instruction: "人物・ロゴ・重要背景は変えず、文字を一切生成せずに背景・装飾・質感だけを高品質化する。指定テロップは保護領域を避けてブラウザで正確に合成する。"
    },
    compositePlan: {
      restore: ["顔", "目", "口", "髪型", "公式ロゴ", "協業ロゴ", "重要背景"],
      faceOverlapPolicy: "顔・目へのテロップ被りを許可しない",
      captionPolicy: "caption_safe_area_no_overlap"
    },
    textPolicy: { primary: "browser_exact_caption", fallback: "browser_exact_caption", requiredCheck: "human_visual_check" }
  };
}
