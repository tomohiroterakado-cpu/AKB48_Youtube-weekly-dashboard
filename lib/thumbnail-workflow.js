...
function normalizeEditRegions(regions) {
  if (!Array.isArray(regions) || !regions.length) throw Object.assign(new Error("編集する範囲をブラシでなぞってください。"), { status: 400 });
  const normalized = regions.map((region, index) => {
    const brushSize = Number(region?.brushSize);
    ...
    if (!Number.isFinite(brushSize) || brushSize < 0.01 || brushSize > 0.4 || !points.length || !points.every(validPoint)) {
      throw Object.assign(new Error(`編集ブラシ ${index + 1} の位置または太さが正しくありません。`), { status: 400 });
    }
    return { brushSize, points: points.map((point) => ({ x: Number(point.x), y: Number(point.y) })) };
  });
  const bounds = normalized.map((region) => {
    const padding = region.brushSize / 2;
    return {
      left: Math.max(0, Math.min(...region.points.map((point) => point.x)) - padding),
      right: Math.min(1, Math.max(...region.points.map((point) => point.x)) + padding),
      top: Math.max(0, Math.min(...region.points.map((point) => point.y)) - padding),
      bottom: Math.min(1, Math.max(...region.points.map((point) => point.y)) + padding)
    };
  });
  const connected = new Set([0]);
  for (let changed = true; changed;) {
    changed = false;
    bounds.forEach((region, index) => {
      if (connected.has(index)) return;
      const touchesConnectedRegion = [...connected].some((connectedIndex) => {
        const other = bounds[connectedIndex];
        return region.left <= other.right && region.right >= other.left && region.top <= other.bottom && region.bottom >= other.top;
      });
      if (touchesConnectedRegion) {
        connected.add(index);
        changed = true;
      }
    });
  }
  if (connected.size !== normalized.length) throw Object.assign(new Error("テロップが欠けないよう、編集ブラシはひと続きの範囲としてなぞってください。"), { status: 400 });
  return normalized;
}
...
function createThumbnailReview(input) {
  ...
  const editRegions = normalizeEditRegions(input?.editRegions || []);
  return {
    workflowVersion: "2.0",
    jobId,
    ...
    editing: {
      mode: "brush_only",
      editRegions,
      outsideBrushStrategy: "keep_original_pixels",
      textStrategy: "local_canvas_exact_japanese"
    },
    candidates: CANDIDATES.map((candidate) => ({
      ...candidate,
      recommendedCopy: requestedCopy,
      generationScope: ["テロップ", "装飾", "金属感", "光沢", "背景プレート"],
      prohibitedScope: ["ブラシで指定していない全領域", "人物の顔", "目", "口", "髪型", "公式ロゴ", "協業ロゴ"]
    })),
...
function selectThumbnailCandidate(review, candidateId) {
  ...
  return {
    ...
    images2Brief: {
      ...
      editRegions: review.editing.editRegions,
      instruction: "ブラシで塗った範囲だけをテロップ・装飾・質感の編集対象にする。未指定領域は元画像のピクセルをそのまま保持し、指定テロップはアプリ側で正確に合成する。"
    },
    compositePlan: {
      strategy: "元画像を土台に、ブラシマスク内だけを編集レイヤーで置き換える",
      outsideMask: "original_pixels_only",
      faceOverlapPolicy: "ブラシ外の顔・目へのテロップ被りを許可しない"
    },
    textPolicy: { primary: "local_canvas_exact_japanese", requiredCheck: "human_visual_check" }
  };
}
...
function selectThumbnailPreviewCandidates(review, candidateIds) {
  if (!Array.isArray(candidateIds) || candidateIds.length !== 2 || new Set(candidateIds).size !== 2) {
    throw Object.assign(new Error("比較する候補を重複なく2案選んでください。"), { status: 400 });
  }
...
function selectAllThumbnailPreviewCandidates(review) {
  if (!review || review.status !== "awaiting_selection") throw Object.assign(new Error("選択待ちの候補を指定してください。"), { status: 400 });
  return (review.candidates || []).map((candidate) => selectThumbnailCandidate(review, candidate.id));
}
...
module.exports = {
  createThumbnailReview,
  selectThumbnailCandidate,
  selectThumbnailPreviewCandidates,
  selectAllThumbnailPreviewCandidates,
