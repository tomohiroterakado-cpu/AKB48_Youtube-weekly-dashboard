...
const GENERATION_RESERVATION_TTL_MS = 10 * 60 * 1000;

function duplicateGenerationError() {
  return Object.assign(new Error("同じ元画像・テロップ・候補は24時間以内に生成済みです。再生成する場合は、テロップまたは保護範囲を変更してから6案を作成してください。"), { status: 409 });
}
...
function decode(value) {
  ...
  catch { throw Object.assign(new Error("サムネイル候補の確認情報が正しくありません。もう一度6案を作成してください。"), { status: 400 }); }
}
...
function verifyThumbnailReview(token, secret, now = Date.now()) {
  ...
  if (!valid) throw Object.assign(new Error("サムネイル候補の確認情報が正しくありません。もう一度6案を作成してください。"), { status: 400 });
  const session = decode(payload);
  if (!session?.review || Number(session.expiresAt) < now) throw Object.assign(new Error("候補の確認時間が切れました。もう一度6案を作成してください。"), { status: 410 });
  return session.review;
}

function generationFingerprint({ review, candidateId, originalImage, variant = "final" }) {
  const source = {
...
class ThumbnailGenerationGuard {
  ...
  reserve(fingerprint) {
    ...
    if (this.entries.has(fingerprint)) throw duplicateGenerationError();
...
function reservePersistentGeneration(state, fingerprint, now = Date.now()) {
  ...
  const existing = state.thumbnailGenerations.find((item) => item.fingerprint === fingerprint);
  const reservedAt = Date.parse(existing?.createdAt || "");
  if (existing?.status === "reserved" && Number.isFinite(reservedAt) && reservedAt + GENERATION_RESERVATION_TTL_MS <= now) {
    state.thumbnailGenerations = state.thumbnailGenerations.filter((item) => item.fingerprint !== fingerprint);
  } else if (existing) {
    throw duplicateGenerationError();
  }
...
function completePersistentGeneration(state, fingerprint, now = Date.now()) {
  const item = (state.thumbnailGenerations || []).find((entry) => entry.fingerprint === fingerprint);
