...
function enrichNarrative(report, values) {
  const weeks = report.weeks.map((week) => {
    ...
    const trendSections = hasPrevious ? [
      { label: "前週比（参考値）", note: "比較対象: 1週", title: "伸びた指標", items: positives },
      { label: "前週比（要確認）", note: "優先改善", title: caution.length ? "来週までに確認する指標" : "大きな悪化は見られません", items: caution.length ? caution : [metrics.subscribers] },
      { label: "月間平均・月間比較", note: "準備中", title: "4週分の蓄積後に自動表示", description: "現在は2週分のため、月間平均・前月比較を断定せず保留します。あと2週分の取込完了後、同じ場所に自動で追加されます。", items: [] }
    ] : [{ label: "比較データ", note: "準備中", title: "前週比は次週から表示", description: "前週比は2週目、月間平均・月間比較は4週目から自動表示します。", items: [] }];
...
async function buildReportWithLegacyGoals(repository, report) {
  ...
  return enrichNarrative(enrichGoals(report, values), values);
}
module.exports = { buildReportWithLegacyGoals, enrichGoals, enrichNarrative };
