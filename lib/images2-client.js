...
function buildImageEditPrompt(production, { hasProtectionMask = false } = {}) {
  ...
  return [
    ...
    "Keep the source image composition and every person exactly as in the source. Do not add, remove, duplicate, rearrange, crop, replace, or alter people, faces, eyes, mouths, hairstyles, hands, bodies, uniforms, partner logos, or important background.",
    hasProtectionMask ? "An alpha mask is supplied: its opaque regions are locked original assets. Preserve those regions faithfully and edit only transparent regions." : "No mask is available, so preserve people and logos solely from the source image and these instructions.",
    ...
    "The typography must feel like a premium commercial Japanese YouTube thumbnail, not a flat banner. Keep all faces and eyes free from important text. Edit typography and non-human background graphics only.",
    "Do not invent additional Japanese copy. Do not change existing official logos. No watermark."
  ].join("\n");
}

async function generateImages2Design({ originalImage, protectionMask, production, outputSize, fetchImpl = fetch, apiKey = process.env.OPENAI_API_KEY, model = process.env.OPENAI_IMAGE_MODEL || "gpt-image-2" }) {
  ...
  const mask = protectionMask ? dataUrlToBlob(protectionMask) : null;
  if (mask && mask.type !== "image/png") throw Object.assign(new Error("人物保護マスクはPNG形式で作成してください。"), { status: 400 });
  if (mask && source.type !== mask.type) throw Object.assign(new Error("元画像と人物保護マスクの形式が一致していません。もう一度画像を選択してください。"), { status: 400 });
  const form = new FormData();
  form.append("model", model);
  form.append("prompt", buildImageEditPrompt(production, { hasProtectionMask: Boolean(mask) }));
  form.append("image[]", new Blob([source.bytes], { type: source.type }), `original.${source.type.split("/")[1]}`);
  if (mask) form.append("mask", new Blob([mask.bytes], { type: mask.type }), "protection-mask.png");
  ...
  return { imageDataUrl: `data:image/png;base64,${image}`, model, outputSize: requestedSize, prompt: buildImageEditPrompt(production, { hasProtectionMask: Boolean(mask) }), usage: payload.usage || null };
}

module.exports = { IMAGE_DIMENSION_STEP, MAX_SOURCE_IMAGE_BYTES, alignImageDimension, normalizedOutputSize, dataUrlToBlob, buildImageEditPrompt, generateImages2Design };
