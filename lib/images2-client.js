...
function buildImageEditPrompt(production, captionSafeArea) {
  ...
  const safeAreaInstruction = safeArea
    ? `Reserve this normalized caption-safe area for a browser compositor: x=${safeArea.x.toFixed(4)}, y=${safeArea.y.toFixed(4)}, width=${safeArea.w.toFixed(4)}, height=${safeArea.h.toFixed(4)}. Remove only existing lettering inside this area while preserving its original non-text visual background, color treatment, decorative shapes, and any pre-existing caption panel or band. If this area has no original caption panel or band, keep the original background unchanged and do not add a banner, panel, empty box, or decorative object. Do not place faces, logos, or new lettering inside this area.`
    : "Reserve a clear rectangular caption-safe area away from faces, logos, and important visual subjects. Preserve the source background and do not add a banner, panel, empty box, or decorative object there.";
  return [
    ...
    "Outside the reserved caption-safe area, do not render, replace, add, stylize, crop, or alter any Japanese or Latin text, numbers, caption plates, labels, logos, or watermarks.",
    safeAreaInstruction,
    "Inside the reserved caption-safe area, keep the source's non-text visual treatment as instructed. A browser compositor will place the exact Japanese caption after this edit.",
