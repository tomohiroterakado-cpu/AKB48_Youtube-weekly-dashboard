...
function buildImageEditPrompt(production) {
  ...
  const brushSummary = editRegions.map((region, index) => {
    const path = (region.points || []).map((point) => `(${point.x}, ${point.y})`).join(" -> ");
    return `brush ${index + 1}: ${path}, width ${region.brushSize}`;
  }).join("; ");
  return [
    ...
    `Only prepare visual treatment inside these normalized brush paths: ${brushSummary}.`,
    "The app will use your result only through that brush mask. Every pixel outside it must remain visually compatible with the original image.",
