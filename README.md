...
取込成功後は、AI Director用テーブルに加え、既存レポートが参照する `CSV_週次集計`、`CSV_貼付用`、`自チャンネル動画`、`CSV_日別` へ同期します。`チャンネル登録者`、`推定収益 (JPY)`、`コメントの追加回数` を含むCSV指標も同期対象です。シートに対応列がない場合は、安全に末尾へ追加します。`メンバーシップ会員数` と `メンバーシップ増減数` は手入力項目です。新しい週の行が表示された後に入力してください。この同期では、既に入力した手動値を消去・上書きしません。
...
「今週の牽引コンテンツ」は、動画IDで重複しない視聴回数上位4本を毎週同じ表示項目（公開日、動画尺、平均視聴時間、再生・反応・登録・収益・CTR）で表示します。値がCSVにない場合は `-` と表示し、項目の並びは変わりません。
