...
6案の比較、保護範囲の指定、品質確認は追加料金なしで利用できます。画像を生成するボタンを押した時だけOpenAI APIの画像編集リクエストが送信され、利用料金が発生する場合があります。同じ元画像・テロップ・候補は24時間、`AI_thumbnail_generations` に記録して二重生成を防ぎます。Cloud Runには `OPENAI_API_KEY` をSecret Managerまたは環境変数で設定し、キーをGitHub・スプレッドシート・画面上へ保存しません。画像編集では元画像をOpenAI APIへ送信するため、公開前素材や権利上扱いに注意が必要な素材は、社内の運用ルールに従って利用してください。

画像AIが安全判定で停止した場合は、画面に **AIなしでテロップを合成する** ボタンが表示されます。この代替モードは、元画像を一切加工せず、指定したテロップだけを合成します。保護範囲に指定した顔・ロゴも元画像のまま保持し、外部AIの追加利用料金は発生しません。
