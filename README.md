...
- 「実績・話題性」へ入力した確認済み事実を使い、実績・事実→感情→内容説明の三層フックを設計。根拠のない再生数や実績は生成しない
- 30案を三層フック、実績・事実先行、感情先行、舞台裏、人物ドラマ、意外性、検索・シリーズの7訴求軸へ固定配分
...
画面上部の **サムネイル制作** では、元画像を保ちながらテロップ・装飾の方向性を6案比較できます。全案で、入力したテロップ文言をそのまま使用します。

1. 元サムネイルを選び、変更したいテロップを入力する。
2. 修正・追加したいテロップが収まる範囲だけをブラシでなぞる。なぞらない領域は元画像のピクセルをそのまま保持する。
3. 6案から1案を選び、必要な場合だけ **選択案をImages2.0で生成** を実行する。
4. 顔・ロゴ・日本語・スマホでの読みやすさを確認し、最終PNGを書き出す。

6案の比較、ブラシ範囲の指定、品質確認は追加料金なしで利用できます。画像を生成するボタンを押した時だけOpenAI APIの画像編集リクエストが送信され、利用料金が発生する場合があります。同じ元画像・テロップ・編集ブラシ・候補は24時間、`AI_thumbnail_generations` に記録して二重生成を防ぎます。Cloud Runには `OPENAI_API_KEY` をSecret Managerまたは環境変数で設定し、キーをGitHub・スプレッドシート・画面上へ保存しません。画像編集では元画像をOpenAI APIへ送信するため、公開前素材や権利上扱いに注意が必要な素材は、社内の運用ルールに従って利用してください。

画像AIが安全判定で停止した場合は、画面に **AIなしでテロップを合成する** ボタンが表示されます。この代替モードは、元画像を一切加工せず、ブラシでなぞった範囲だけに指定テロップを合成します。ブラシ外の領域は元画像のまま保持し、外部AIの追加利用料金は発生しません。
...
取込成功後は、AI Director用テーブルに加え、既存レポートが参照する `CSV_週次集計`、`CSV_貼付用`、`自チャンネル動画`、`CSV_日別` へ同期します。`チャンネル登録者`、`推定収益 (JPY)`、`コメントの追加回数` を含むCSV指標も同期対象です。シートに対応列がない場合は、安全に末尾へ追加します。`手入力_メンバーシップ会員数_締日時点` は手入力項目です。新しい週の行が表示された後に入力してください。レポートではこの会員数を主表示し、前週の締日時点との増減を小さく表示します。この同期では、既に入力した手動値を消去・上書きしません。
...
確認済み動画も「マスタ管理」の動画属性から、企画ジャンル、サブジャンル、出演メンバー、ゲスト、タグ、コラボ、タイトル訴求、想定ターゲット、季節イベント、制作コスト、撮影難易度、公開設定、備考を後から編集できます。`公開設定` を `限定公開` または `非公開` にした動画は「今週の牽引コンテンツ」候補から除外されます。CSVに公開設定列がある場合は自動で取り込み、列がない場合も一度設定すれば以後の更新で保持されます。

「今週の牽引コンテンツ」は、動画IDで重複しない視聴回数上位4本を表示します。公開日、動画尺、平均視聴時間を加え、ジャンルは表示しません。CSVにない数値は表示せず、カードの情報量を必要以上に増やしません。
