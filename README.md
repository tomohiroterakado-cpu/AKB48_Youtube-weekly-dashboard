...
取込成功後は、AI Director用テーブルに加え、既存レポートが参照する `CSV_週次集計`、`CSV_貼付用`、`自チャンネル動画`、`CSV_日別` へ同期します。`メンバーシップ会員数` と `メンバーシップ増減数` は手入力項目です。新しい週の行が表示された後に入力してください。この同期では、既に入力した手動値を消去・上書きしません。
